...
- Louvain 社区发现，以 3–6 条分支为目标；论文稀疏时可能更少
- Birth、Breakthrough、Branching、Revival、Translation、Challenge、Correction 规则事件
- Workers AI 根据所选论文为分支和事件生成文案，并根据已有时间线提出待核查问题；应用严格校验证据，失败重试一次，未通过的部分保留规则文案
...
手机回放操作可折叠，模式和分享入口均可使用。当前浏览器会话中的重复查询复用自己的回放，失败任务可以显式重试；超出创建或重试额度时显示提示。

## 数据与隐私

回放默认非公开列出（unlisted），链接含随机标识，需要持有链接才能读取；再次输入同一主题不会找回其他浏览器的回放。无登录、无 Cookie。生成的回放与运行记录保存在 Cloudflare D1 数据库中：失败任务保留 7 天、成功回放保留 90 天后自动删除。当前标签页保存随机复用凭据，用于恢复本会话相同查询，不发送给文献服务或 AI。为防滥用，原始 IP 仅用于 D1 限流；超过 24 小时的限流记录在下一次定时或流量清理时删除，不写入错误日志。主题会发送给 OpenAlex，所选论文和主题会送给 Workers AI；摘要和 DOI 查询分别发送给 Europe PMC、Crossref。请勿在主题中输入可识别患者身份的信息。

## 本地运行

要求 Node.js 22.13 或更高版本，以同时满足 Wrangler 开发/部署工具和内置 SQLite 数据库验证。
...
- `Challenge` 是机器检测的待核查候选；`Correction` 仅采用明确针对该论文的更正、撤稿、关注或恢复状态，并按来源更新日期定位；没有更新日期时只在证据抽屉显示状态，不推断学术不端
