...
手机回放操作可折叠，模式、语言和分享入口均可用；修正页首换行与播放栏对比。公开创建和重试接口增加每小时 IP 限流，相同查询复用已有回放。

限流使用 D1 的 `rate_limits` 表，按端点和客户端 IP 分桶。配置项为 `RATE_LIMIT_CREATE_PER_HOUR` 与 `RATE_LIMIT_RETRY_PER_HOUR`；非法值回退默认。存储故障时记录告警并放行，属于可用性优先的策略。现有数据库更新需包含迁移 `0003_query_hash_unique.sql` 和 `0004_rate_limits.sql`；第三个迁移会清理重复查询对应的回放，优先保留已完成记录，同级保留最早记录。正式应用迁移前应备份数据库并核对重复数据。2026-09-13 已备份现有数据库并应用两项迁移，升级后原有 9 个回放均保留。新环境仍需先完成迁移再部署代码。
