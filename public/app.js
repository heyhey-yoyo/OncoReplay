// 仅用于在当前浏览器会话内复用相同查询；不发送到文献服务或 AI。
let replayReuseKey;
try {
  replayReuseKey = sessionStorage.getItem('oncoreplay-reuse-key');
  if (!/^[a-f0-9]{32}$/.test(replayReuseKey || '')) {
    replayReuseKey = crypto.randomUUID().replaceAll('-', '');
    sessionStorage.setItem('oncoreplay-reuse-key', replayReuseKey);
  }
} catch { replayReuseKey = crypto.randomUUID().replaceAll('-', ''); }
import {
  clamp,
  currentEvent,
  escapeHtml,
...
async function createCustomReplay(payload,button) {
  ...
  try {
    const response=await fetch('/api/replays',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...payload,locale:state.locale,reuseKey:replayReuseKey})});
    const result=await response.json().catch(()=>({}));
...
function handleReplayResize(){if(innerWidth>760){const actions=document.querySelector('.replay-actions');if(actions)actions.open=true;}clearTimeout(state.resizeTimer);state.resizeTimer=setTimeout(()=>state.replay&&renderTimeline(state.replay),130);}
...
function renderTimeline(replay) {
  ...
  const debateMode=state.mode==='debate';
  const edges=(replay.edges||[]).map(edge=>{const source=workMap.get(edge.source),target=workMap.get(edge.target);if(!source||!target)return'';const visible=source.publicationYear<=state.currentYear&&target.publicationYear<=state.currentYear;const x1=x(source.publicationYear),y1=y(source.branchId),x2=x(target.publicationYear),y2=y(target.branchId),cx=(x1+x2)/2;return `<path class="citation-edge ${edge.type==='related'?'bridge':''}${debateMode?' debate':''}" style="opacity:${visible ? (debateMode && edge.type !== 'related' ? .25 : 1) : 0}" d="M${x1} ${y1} C${cx} ${y1},${cx} ${y2},${x2} ${y2}"/>`;}).join('');
  const stacks=new Map();replay.works.forEach(work=>{const key=`${work.branchId}|${work.publicationYear}`,arr=stacks.get(key);arr?arr.push(work):stacks.set(key,[work]);});
  const offsets=new Map();stacks.forEach(group=>{group.sort((a,b)=>nodeWeight(b,state.mode)-nodeWeight(a,state.mode));const spacing=Math.min(9,Math.max(4,branchGap*0.8/Math.max(1,group.length-1)));group.forEach((work,i)=>offsets.set(work.id,(i-(group.length-1)/2)*spacing));});
  const nodes=replay.works.map(work=>{const px=x(work.publicationYear),py=y(work.branchId)+(offsets.get(work.id)||0),size=nodeWeight(work,state.mode),future=work.publicationYear>state.currentYear,signal=debateMode?work.debateSignal:work.normalizedImpact,dimmed=signal<.45,debateRing=debateMode&&Number(signal)>=.55?`<circle class="debate-ring${Number(signal)>=.8?' hot':''}" cx="${px}" cy="${py}" r="${size+3.5}"/>`:'',branch=replay.branches.find(b=>b.id===work.branchId),color=getBranchColor(branch,replay.branches.indexOf(branch),replay),shape=nodeShape(work.workType,px,py,size,color);return `<g class="work-node ${future?'hidden-future':''} ${dimmed?'dimmed':''}" data-work-id="${work.id}" style="opacity:${future ? .03 : nodeOpacity(work,state.mode)}"><title>${escapeHtml(work.title)}</title>${shape}${debateRing}${work.updateStatus||work.isRetracted?`<path d="M${px+size*.55} ${py-size*.7}h5l-2.5 4.5Z" fill="#ff776e"/>`:''}</g>`;}).join('');
