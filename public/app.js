...
const builtinTranslations = {
  ...
  events: {
    ...
    'event-05': ['结构指导的化学研究明显升温','药物发现记录变得更密集，也与早期机制工作连接得更紧。引用增长发生在多个相关节点，而非单篇论文。','归一化引用增长和跨分支连接同时上升。'],
    ...
    'event-08': ['多个项目并行加速','机制、化学和转化分支在短时间内同时加入高影响节点，形成领域级加速，而不是单线突破。','多个分支的节点密度和归一化影响同时增长。'],
...
function renderHome() {
  ...
  app.innerHTML = `<div class="shell">${siteHeader()}<main id="main">
    <section class="hero">${heroNetwork()}<div class="container hero-content">
      ...
      <h1 class="display">${L('观看一个肿瘤研究观点如何','Watch a cancer research idea')} <em>${L('演化。','evolve.')}</em></h1>
      <p class="lede">${L('把论文、引用、争议和临床转化组织成一条可以暂停、核查和分享的互动时间线。','Turn papers, citations, controversies, and clinical translation into an interactive timeline you can pause, inspect, and share.')}</p>
    ...
    <section class="trust-strip" aria-label="${L('数据来源','Data sources')}"><div class="container trust-inner"><span>${L('围绕开放学术元数据设计','Designed around open scholarly metadata')}</span><div class="source-logos"><span>OpenAlex</span><span>Europe PMC</span><span>Crossref</span></div></div></section>
    <section class="section"><div class="container"><div class="section-head"><div><span class="eyebrow">${L('精选回放','Curated replays')}</span><h2 class="section-title">${L('从故事进入，而不是面对空白页。','Enter through a story, not a blank screen.')}</h2></div><p class="section-copy">${L('每条回放把叙事、来源元数据和不确定性明确分开。','Each replay keeps narrative, source metadata, and uncertainty visually separate.')}</p></div><div class="cards">
    ...
    <section class="section" style="padding-top:20px"><div class="container"><div class="section-head"><div><span class="eyebrow">${L('产品循环','The product loop')}</span><h2 class="section-title">${L('检索、重建、回放。','Find. Reconstruct. Replay.')}</h2></div><p class="section-copy">${L('AI 只组织来源约束的材料，不生成标识符、不改变发表日期，也不宣布科学共识。','AI organizes source-bound material; it does not invent identifiers, change publication dates, or declare scientific consensus.')}</p></div><div class="steps">
      <article class="step"><span class="step-index">01 / ${L('找到论文','Find the papers')}</span><h3>${L('先确认检索边界。','Start from a checked query.')}</h3><p>${L('生成前预览实体、同义词、年份范围和样本文献。','Preview entities, synonyms, date range, and sample works before generation.')}</p></article>
      <article class="step"><span class="step-index">02 / ${L('重建时间线','Reconstruct the timeline')}</span><h3>${L('评分变化，而非只看总引用。','Score change, not just citation totals.')}</h3><p>${L('组合时间、归一化影响、网络桥接、分支形成、复兴和结构化更新。','Combine time, normalized influence, graph bridges, branches, revival, and structured updates.')}</p></article>
      <article class="step"><span class="step-index">03 / ${L('回放领域','Replay the field')}</span><h3>${L('先看故事，再核查证据。','See the story, then inspect evidence.')}</h3><p>${L('停在任意年份、切换观看镜头，并打开每个事件背后的来源。','Pause at any year, switch lenses, and open the sources behind every event.')}</p></article>
    </div></div></section>
  </main>${footer()}</div>`;
...
function renderCreate() {
  ...
  app.innerHTML = `<div class="shell">${siteHeader()}<main id="main">
    <section class="page-hero"><div class="container"><span class="eyebrow">${L('创建回放','Create replay')}</span><h1>${L('先定义研究边界，再重建领域。','Define the field before reconstructing it.')}</h1><p>${L('在生成前检查实体、同义词、年份覆盖和样本文献。自定义回放会真实调用 OpenAlex、Europe PMC、Crossref、D1、Queues 和 Workers AI。','Review entities, synonyms, date coverage, and sample works. Custom generation uses OpenAlex, Europe PMC, Crossref, D1, Queues, and Workers AI.')}</p></div></section>
    <section class="container create-grid">
      <form class="form-card form-stack" id="create-form">
        ...
        <div class="form-actions"><button class="button primary" type="submit">${L('预览检索','Preview query')} ${icons.arrow}</button><a class="button ghost" href="/replay/kras-g12d" data-nav>${L('使用内置示例','Use the built-in example')}</a></div>
      </form>
      <aside class="preview-card" id="query-preview"><div class="preview-empty"><div><div class="orb"></div><strong>${L('检索预览','Query preview')}</strong><p>${L('实体建议、覆盖范围和样本文献会显示在这里。','Entity suggestions, coverage, and sample works will appear here.')}</p></div></div></aside>
    </section>
  </main>${footer()}</div>`;
  bindCommonNavigation();
...
async function handleQueryPreview(event) {
  ...
  preview.innerHTML = `<div class="preview-loading"><span class="eyebrow">${L('检索预览','Query preview')}</span><h3 style="font-size:24px;margin:20px 0">${L('正在核对研究边界','Checking the research boundary')}</h3>${pipelineMarkup(0)}</div>`;
  let active=0;
  const interval=setInterval(()=>{ active=Math.min(3,active+1); const loading=preview.querySelector('.preview-loading'); if(loading) loading.innerHTML=`<span class="eyebrow">${L('检索预览','Query preview')}</span><h3 style="font-size:24px;margin:20px 0">${L('正在核对研究边界','Checking the research boundary')}</h3>${pipelineMarkup(active)}`; },320);
  try {
...
function pipelineMarkup(active) {
  const steps = state.locale === 'zh' ? ['标准化主题','识别实体和同义词','抽样学术记录','估算年份覆盖'] : ['Normalize topic','Identify entities and synonyms','Sample scholarly records','Estimate date coverage'];
...
const stageLabels={
  FETCH_WORKS:['检索论文并扩展引用网络','Finding works and expanding citations'],
  ENRICH_BIOMEDICAL:['补充 Europe PMC 摘要并核对 Crossref 更新','Enriching abstracts and checking updates'],
  BUILD_TIMELINE:['计算 turning-point 并执行 Louvain 聚类','Scoring turning points and running Louvain clustering'],
  GENERATE_NARRATIVE:['生成严格 Schema 的来源约束叙事','Writing schema-validated source-bound narratives'],
  FINALIZE_REPLAY:['整理可视化数据并发布回放','Finalizing the replay'],
};
